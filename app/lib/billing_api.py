...
class BaseBillingApi(object):
    ...
    def create_api(enterprise):
        if ('CCEAccounts' == enterprise.billing_method):
            return EAccountsBillingApi()
        elif ('Stripe' == enterprise.billing_method):
            return StripeBillingApi()

    """
    @abstractmethod
    def create_account(self, billing):
        pass
    """
...
class StripeBillingApi(BaseBillingApi):
        ...
        try:
            ...
            for oi in order.active_items:
                ...
                if p.subscription:
                    ...
                    stripe_cust.update_subscription(plan=p.sku,
                                                    coupon=self.coupon)

                else:
                    invoice = stripe.InvoiceItem.create(
                        customer=stripe_cust.id,
                        amount=campaign.get_product_price(p),
                        currency="usd",
                        description=p.name)
            return True
        except stripe.CardError as e:
            self.last_staus = e.code
            self.last_note = e.message
        except Exception as e2:
            self.last_note = e2.message
        return False
    ...
    def cancel_order(self, order, billing):
        try:
            cust = order.customer
            if not cust.third_party_id: return False
            stripe.api_key = cust.campaign.company.enterprise.get_attr('stripe_private_key')
            stripe_cust = stripe.Customer.retrieve(cust.third_party_id)
            for oi in order.active_items:
                p = oi.product
...
class EAccountsBillingApi(BaseBillingApi):
    ...
    def _create_history(self, resp, order, billing, customer):
        from app.model.crm.billing import BillingHistory
