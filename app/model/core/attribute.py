...
class Attribute(ORMBase, BaseModel):
    ...
    def find_values(obj):
        ...
        retval = {}
        for av in avs:
            retval[av.attribute.attr_name] = av.attr_value
        return retval
    ...
    def set(self, obj, value):
        av = AttributeValue()
        av.attribute = self
        av.fk_type = self.fk_type
        av.fk_id = getattr(obj, obj.__pk__)
        av.attr_value = value
        av.save()
        return av

    def get(self, obj):
        av = AttributeValue.find(self, getattr(obj, obj.__pk__))
        return av.attr_value if av else None

    @staticmethod
    def clear_all(fk_type, fk_id):
        avs = AttributeValue.clear_all(fk_type, fk_id)
